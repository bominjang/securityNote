...
#### C++ 분석
    ...
    0x00401069 dword ptr -88h

    0x00401084 mov [ebp+var_88], 1111h

    0x004010A9 lea ecx, [ebp+var_88]

    0x004010AF call sub_401000

var_88 은 클래스 혹은 구조체 일 것이다. 그렇다면 당연히 해당 메모리 영역에 값을 넣을 것. 0x401084 번지를 보면 var_88번지에 직접 0x1111을 넣고 있음. 해당 객체의 첫 번째 멤버 변수가 되며, ``` 인스턴스명.변수명 = 0x1111; ```에 해당하는 코드일 것. 0x4010A9 코드를 보면, 해당 객체의 변수에 해당하는 var_88의 번지를 ecx에 넣고 sub_401000 함수를 호출하고 있음. 보통 클래스의 멤버 함수를 호출할 때는 현재 메모리 덩어리의 포인터를 ecx에 넣고 함수 호출을 하므로 이런 코드로 클래스임을 짐작할 수 있음. 어쨌던 ecx에 들어간 var_88은 당연히 C++ 에서의 this 포인터가 됨. 이처럼 클래스 멤버 변수는 ecx에 객체의 포인터를 넣고 함수 호출을 하는 형태가 일반적. 따라서 함수 호출 직전 ecx에 어떤 값을 넣는 모습을 눈여겨 봐야 함.이런 루틴이 왔을 때 한눈에 클래스 멤버 함수라는 것을 알아채야 함. ecx에는 this 포인터가 전달돼 왔으므로 이를 염두에 두고 ShowData를 살폅자]
